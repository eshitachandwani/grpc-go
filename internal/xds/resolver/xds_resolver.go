...
func (r *xdsResolver) newConfigSelector() (_ *configSelector, err error) {
	...
	for i, rt := range r.xdsConfig.VirtualHost.Routes {
		...
		if rt.ClusterSpecifierPlugin != "" {
			...
			if !ok {
				// Take a reference on behalf of this config selector. It is
				// released by cs.stop(), including via the deferred call above
				// if a later route fails to build.
			...
			for _, wc := range rt.WeightedClusters {
				...
				if !ok {
					// Take a reference on behalf of this config selector. It is
					// released by cs.stop(), including via the deferred call

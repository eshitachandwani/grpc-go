...
var (
	...
	// newChildBalancer is a helper function to build a new priority balancer and will be overridden in unittests.
	newChildBalancer = func(cc balancer.ClientConn, opts balancer.BuildOptions) (balancer.Balancer, error) {
		...
		// priority balancer because the cdsBalancer does not deal with
		// subConns.
...
func (bb) Build(cc balancer.ClientConn, opts balancer.BuildOptions) balancer.Balancer {
	...
	b := &cdsBalancer{
		...
		prioritymap:       make(map[discoveryMechanismKey]priorityConfig),
...
// priority balancer to handle . Implements the balancer.Balancer
// interface which is exposed to gRPC and implements the balancer.ClientConn
// interface which is exposed to the priority balancer.
type cdsBalancer struct {
	...
	prioritymap map[discoveryMechanismKey]priorityConfig
...
// clusterName), xdsConfig which has all the resolved resources and the xdsClient object from the
// xdsResolver.
func (b *cdsBalancer) UpdateClientConnState(state balancer.ClientConnState) error {
	...
	callback := func(context.Context) {
		...
		if err := b.handleSecurityConfig(b.clusterConfig[b.lbCfg.ClusterName].Config.Cluster.SecurityCfg); err != nil {
			...
			b.onClusterError(b.lbCfg.ClusterName, b.annotateErrorWithNodeID(fmt.Errorf("received Cluster resource contains invalid security config: %v", err)))
			errCh <- nil
			return
		}
		b.onClusterUpdate()
...
func (b *cdsBalancer) onClusterUpdate() {
	...
	case xdsresource.ClusterTypeEDS:
		...
		if _, ok := b.prioritymap[dmkey]; !ok {
...
//
// Only executed in the context of a serializer callback.
// func (b *cdsBalancer) generateDMsForCluster(name string, depth int, dms []priority.DiscoveryMechanism, clustersSeen map[string]bool) ([]priority.DiscoveryMechanism, bool, error) {
...
// 	var dm priority.DiscoveryMechanism
...
// 	case xdsresource.ClusterTypeEDS:
// 		dm = priority.DiscoveryMechanism{
...
// 	case xdsresource.ClusterTypeLogicalDNS:
// 		dm = priority.DiscoveryMechanism{
// 			Type:                  priority.DiscoveryMechanismTypeLogicalDNS,
// 			Cluster:               cluster.ClusterName,

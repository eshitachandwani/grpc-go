...
func newTestContextWithHandshakeInfo(parent context.Context, root, identity certprovider.Provider, sanExactMatch string, sni string, autoSniSanValidation bool) context.Context {
...
func (s) TestClientCredsHandshakeFailure(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			sniEnvVarEnabled:     true,
		},
		{
			desc:                 "SNI set SAN mismatch",
			handshakeFunc:        testServerTLSHandshake,
			rootProvider:         makeRootProvider(t, "x509/server_ca_cert.pem"),
		...
		{
			desc:                 "SAN mismatch with SNI set and auto SNI SAN validation enabled but environment variable disabled",
			handshakeFunc:        testServerTLSHandshake,
			rootProvider:         makeRootProvider(t, "x509/server_ca_cert.pem"),
			sni:                  defaultTestCertSAN,
			san:                  "bad-san",

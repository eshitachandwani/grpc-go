...
// dnsWatcher watches updates for the given DNS hostname.
//
...
func newDNSResolver(target string, parent *XdsDependencyManager) *dnsWatcher {
	...
	if err != nil {
		if logger.V(2) {
			logger.Infof("Failed to parse dns hostname %q in clusterresolver LB policy", target)
		}
	...
	if err != nil {
		if logger.V(2) {
			logger.Infof("Failed to build DNS resolver for target %q: %v", target, err)
		}
		ret.updateReceived = true
		handleUpdate := func(context.Context) { ret.parent.onDnsResourceError(ret.target, err) }
		ret.parent.Serializer.ScheduleOr(handleUpdate, func() {})
...
// func (dr *dnsWatcher) lastUpdate() (any, bool) {
// 	// dr.mu.Lock()
// 	// defer dr.mu.Unlock()
...
// interface passed to it (implemented by dnsWatcher in this case)
...
func (dr *dnsWatcher) stop() {
	if dr.dnsR != nil {
		dr.dnsR.Close()
	}
}

// dnsWatcher needs to implement resolver.ClientConn interface to receive
// updates from the real DNS resolver.

func (dr *dnsWatcher) UpdateState(state resolver.State) error {
	if logger.V(2) {
		logger.Infof("DNS discovery mechanism for resource %q reported an update: %s", dr.target, pretty.ToJSON(state))
	}
...
func (dr *dnsWatcher) ReportError(err error) {
	if logger.V(2) {
		logger.Infof("DNS discovery mechanism for resource %q reported error: %v", dr.target, err)
	}
...
func (dr *dnsWatcher) NewAddress(addresses []resolver.Address) {
	dr.UpdateState(resolver.State{Addresses: addresses})
}

func (dr *dnsWatcher) ParseServiceConfig(string) *serviceconfig.ParseResult {
